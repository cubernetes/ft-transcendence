...
import { CreateTournamentDTO, TournamentIdDTO, TournamentNameDTO } from "./tournament.types.ts";
...
export const createTournamentHandler = async (
    ...
    try {
        ...
        if (!tournament) {
            return reply.code(400).send({ error: "Failed to create tournament" });
        }
...
export const getTournamentByIdHandler = async (
    ...
    try {
        ...
        req.log.error({ error }, "Failed to get tournament by ID");
        return reply.code(500).send({ error: "Internal server error" });
    }
};
...
export const getTournamentByNameHandler = async (
    ...
    try {
        ...
        req.log.error({ error }, "Failed to get tournament by tournamentname");
        return reply.code(500).send({ error: "Internal server error" });
    }
};
...
export const getAllTournamentsHandler = async (req: FastifyRequest, reply: FastifyReply) => {
    try {
        ...
        req.log.error({ error }, "Failed to get all tournaments");
        return reply.code(500).send({ error: "Internal server error" });
    }
};
