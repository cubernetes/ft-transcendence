...
export async function mainMenu(): Promise<void> {
	...
	// Return to the menu after the game finishes
	mainMenu();
}
...
async function startRemoteGame() {
	try {
		...
		startKeyListener((dir) => {
			...
			if (wsManager) { wsManager.sendDirection(`move ${dir}`); }
		});
		
		// Wait for the game to end
		await waitForGameEnd();
		console.log("Game Over. Returning to menu...");
	} catch (err) {
...
async function waitForGameEnd(): Promise<void> {
	// Simulate the game end (this could be triggered by user input or server logic)
	await inquirer.prompt([
		{
			type: 'confirm',
			name: 'gameOver',
			message: 'The game has ended. Return to the menu?',
			default: true
		}
	]);
}
