...
until curl --silent "http://elasticsearch:${ELASTICSEARCH_PORT:-9200}/_cluster/health" | grep -q '"status":"green"\|"status":"yellow"'; do
...
curl -X PUT "http://elasticsearch:${ELASTICSEARCH_PORT:-9200}/_ilm/policy/ft-transcendence-logs-policy" \
...
curl -X PUT "http://elasticsearch:${ELASTICSEARCH_PORT:-9200}/_template/ft-transcendence-logs" \
  --header 'Content-Type: application/json' \
  --data '{
...
curl -X PUT "http://elasticsearch:${ELASTICSEARCH_PORT:-9200}/%3Cft-transcendence-logs-%7Bnow%2Fd%7D-000001%3E" \
  --header 'Content-Type: application/json' \
  --data '{
    "aliases": {
